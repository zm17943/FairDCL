...
class DatasetTrain(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
       ...
       #                        interpolation=cv2.INTER_NEAREST) # (shape: (new_img_h, new_img_w))
        ...
        ########################################################################
        start_x = np.random.randint(low=0, high=(self.new_img_w - 512))
        end_x = start_x + 512
        #start_y = np.random.randint(low=0, high=(new_img_h - 512))
        #end_y = start_y + 512

        img = img[0:512, start_x:end_x] # (shape: (256, 256, 3))
        label_img = label_img[0:512, start_x:end_x] # (shape: (256, 256))
        ########################################################################
...
class DatasetVal(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return (img, original_label, label_img, img_id)
