...
def evaluate(model, cfg, is_training=False, ckpt_path=None, logger=None):
    ...
    metric_op = er.metric.PixelMetric(cfg.EVAL_DATA_CONFIG['num_classes'], logdir=cfg.SNAPSHOT_DIR, logger=logger)
    with torch.no_grad():
        for ret, ret_gt in tqdm(eval_dataloader):
            ret = ret.to(torch.device('cuda'))
            cls = model(ret)
            cls = cls['out'].argmax(dim=1).cpu().numpy()
...
def evaluate_rural(model, cfg, is_training=False, ckpt_path=None, logger=None):
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.enabled = False
    if cfg.SNAPSHOT_DIR is not None:
    ...
    if not is_training:
        ...
        count_model_parameters(model, logger)
    model.eval()
    print(cfg.EVAL_DATA_RURAL_CONFIG)
    eval_dataloader = LoveDALoader(cfg.EVAL_DATA_RURAL_CONFIG)
    metric_op = er.metric.PixelMetric(cfg.EVAL_DATA_RURAL_CONFIG['num_classes'], logdir=cfg.SNAPSHOT_DIR, logger=logger)
    with torch.no_grad():
        for ret, ret_gt in tqdm(eval_dataloader):
            ret = ret.to(torch.device('cuda'))
            cls = model(ret)
...
def test_target(model, cfg, is_training=False, ckpt_path=None, logger=None):
    #torch.backends.cudnn.deterministic = True
    #torch.backends.cudnn.benchmark = False
    #torch.backends.cudnn.enabled = False
    if cfg.SNAPSHOT_DIR is not None:
    ...
    if not is_training:
        ...
        count_model_parameters(model, logger)
    model.eval()
    logger.info(cfg.TEST_DATA_CONFIG)
    ...
    with torch.no_grad():
        for ret, ret_gt in tqdm(eval_dataloader):
            ...
            # Change here !!! (Not visualizing for now)
            if cfg.SNAPSHOT_DIR is not None:
                for fname, pred in zip(ret_gt['fname'], cls):
                    viz_op(pred, fname.replace('tif', 'png'))
            ...
            ## Visualizing masks
            # if cfg.SNAPSHOT_DIR is not None:
            #     for fname, pred in zip(ret_gt['fname'], cls_gt):
            #         viz_op(pred, fname.replace('tif', 'png'))
...
def test_source(model, cfg, is_training=False, ckpt_path=None, logger=None):
    #torch.backends.cudnn.deterministic = True
    #torch.backends.cudnn.benchmark = False
    #torch.backends.cudnn.enabled = False
    if cfg.SNAPSHOT_DIR is not None:
    ...
    if not is_training:
        ...
        count_model_parameters(model, logger)
    model.eval()
    print(cfg.TEST_DATA_CONFIG_SOURCE)
    eval_dataloader = LoveDALoader(cfg.TEST_DATA_CONFIG_SOURCE)
    metric_op = er.metric.PixelMetric(cfg.EVAL_DATA_CONFIG['num_classes'], logdir=cfg.SNAPSHOT_DIR, logger=logger)
    with torch.no_grad():
        for ret, ret_gt in tqdm(eval_dataloader):
            ret = ret.to(torch.device('cuda'))
